...
class EscKontrol:
    ...
    def hiz_ayarla(self, yuzde):
        ...
        Ondalikli degerler (orn. 7.1, 7.25) de kullanilabilir - pigpio darbe
        genisligini mikrosaniye (tam sayi) cinsinden bekledigi icin en yakin
        mikrosaniyeye yuvarlanir, ama bu adim 100 seviyeye bolununce ~10us'a
        denk geldigi icin ondalikli yuzdeler (orn. 0.1 farki) hala anlamli bir
        gercek darbe farkina karsilik gelir."""
        if not self._hazir:
...
def main():
    ...
    try:
        while True:
            girdi = input("\nHiz yuzdesi (0-100, ondalikli olabilir - orn. 7.1), 'q' ile cikis: ").strip().lower()
            if girdi == "q":
                break
            try:
                yuzde = float(girdi)
