...
# Vcc (5V hatti gerilimi) takibi - guc yetersizligi teshisi icin. Test
# boyunca gorulen en dusuk degeri saklar, sonunda ozet olarak yazdirilir.
...
def vcc_kontrol_et(vcc_mv):
    ...
    if vcc_mv is None:
        return
    if VCC_TAKIP["en_dusuk"] is None or vcc_mv < VCC_TAKIP["en_dusuk"]:
        VCC_TAKIP["en_dusuk"] = vcc_mv
    if vcc_mv < VCC_TAKIP["dusuk_gerilim_esigi"] and not VCC_TAKIP["uyari_verildi"]:
        print(f"  [UYARI] DUSUK GERILIM tespit edildi: {vcc_mv:.0f}mV "
...
def ileri_git_engel_bulunca(bridge, pwm_a, pwm_b, esik_cm=ENGEL_ESIGI_CM,
    ...
    STALL_ESIGI = 1.0  # saniye - mesafe bu sure degismezse stall say (artik tum hiz modlarinda)
    KURTARMA_SURESI = 0.15  # saniye - kurtarma icin tam hizda kalinacak sure
    MAKS_ARDISIK_KURTARMA = 5  # bu kadar ardisik kurtarma basarisiz olursa vazgec, guvenlik amacli dur
    ...
    while time.time() - baslangic < maks_sure:
        ...
        if yeni_ornek_mi and mesafe is not None and mesafe > 0:  # -1 = gecersiz okuma, yoksay
            ...
            # STALL tespiti: HANGI hiz modunda olursa olsun (tam hiz dahil),
            # mesafe STALL_ESIGI suresi boyunca hic degismediyse, robot
            # fiziksel olarak durmus demektir. Kisa bir 'kurtarma' atisi yap.
            # ONCEKI SURUM sadece yavas modlarda kontrol ediyordu - bu da tam
            # hizdayken olusan tam stall'lari (hicbir tekerin donmedigi durum)
            # kacirmasina yol aciyordu.
            if son_mesafe_degeri is None or abs(mesafe - son_mesafe_degeri) >= 1.0:
                son_mesafe_degeri = mesafe
                son_mesafe_degisim_zamani = time.time()
                ardisik_basarisiz_kurtarma = 0
            elif (time.time() - son_mesafe_degisim_zamani > STALL_ESIGI and
                  mesafe > esik_cm):
                ardisik_basarisiz_kurtarma += 1
                print(f"  (STALL tespit edildi - {mesafe:.1f} cm'de takili kaldi, "
                      f"kurtarma atisi yapiliyor... [{ardisik_basarisiz_kurtarma}/{MAKS_ARDISIK_KURTARMA}])")

                if ardisik_basarisiz_kurtarma > MAKS_ARDISIK_KURTARMA:
                    print(f"  HATA: {MAKS_ARDISIK_KURTARMA} kurtarma denemesi de basarisiz oldu - "
                          f"robot gercekten hareket etmiyor. Muhtemelen fiziksel bir baglanti "
                          f"sorunu (motor/kablo/L298N). Guvenlik amacli durduruluyor.")
                    motorlari_durdur(pwm_a, pwm_b)
                    return False
...
def main():
    ...
    # NOT: Otomatik isinma hareketi (kucuk sallanma) KALDIRILDI. EEPROM'dan
    # kalibrasyon zaten yukleniyor ve hizli mod (renk sensoru kapali) veri
    # kalitesini onemli olcude arttirdigi icin, bu hareketin sagladigi kucuk
    # fayda (mag kalibrasyonuna hafif katki), robotun baslangic yonunde
    # istenmeyen kaymaya yol acma riskine degmiyor. Gerekirse yeniden
    # aktiflestirmek icin: isinma_yap(bridge, pwm_a, pwm_b)
    ...
    try:
        ...
        pwm_b.stop()
        GPIO.cleanup()
        if VCC_TAKIP["en_dusuk"] is not None:
            print(f"\n[VCC OZETI] Test boyunca gorulen en dusuk gerilim: "
                  f"{VCC_TAKIP['en_dusuk']:.0f} mV")
        bridge.stop()
